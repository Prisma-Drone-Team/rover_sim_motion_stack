...
class Yolov11Node(Node):
    ...
    def __init__(self):
        ...
        # Subscribers
        self._color_image_sub = self.create_subscription(CompressedImage, "/rover/color/image_raw/compressed", self.color_image_callback, qos_profile_sensor_data, callback_group=self.group_1)
        self._depth_image_sub = self.create_subscription(Image, "/rover/depth/image_raw", self.depth_image_callback, qos_profile_sensor_data, callback_group=self.group_1)
        self._camera_info_subscriber = self.create_subscription(CameraInfo, '/rover/color/camera_info', self.camera_info_callback, QoSProfile(depth=1,reliability=ReliabilityPolicy.RELIABLE), callback_group=self.group_1)
